//  EmotionLongView.swift — Nubi (SF Symbols)
import SwiftUI
...
struct EmotionLogView: View {
            ...
            VStack(spacing: 0) {
                ...
                ZStack {
                    LinearGradient(colors: [Color.nubiLightBlue.opacity(0.4), Color.nubiParchment],
                        ...
                        .ignoresSafeArea(edges: .top).frame(height: 160)
                    VStack(spacing: 6) {
                        Text(selectedPrimary == nil ? "Como te sientes?" : "Cuentame mas...")
                            .font(NubiFont.heading).foregroundColor(.nubiDark)
                        Text(selectedPrimary == nil
                             ? "Selecciona tu emocion principal"
                             : "Que describe mejor tu \(selectedPrimary!.rawValue.lowercased())?")
                ...
                ScrollView(showsIndicators: false) {
                    VStack(spacing: 28) {
                        NubiAvatarView(color: selectedPrimary?.color ?? .nubiLightBlue, size: 100)
            ...
            ForEach(PrimaryEmotion.allCases) { emotion in
                Button {
                    withAnimation(.spring(response: 0.35)) {
                        selectedPrimary = emotion; animateNubi = true
                    }
                    ...
                    .overlay(RoundedRectangle(cornerRadius: 20).stroke(emotion.color.opacity(0.3), lineWidth: 1.5))
                }
        ...
        VStack(spacing: 16) {
            ...
            HStack(spacing: 10) {
                ...
                Text(selectedPrimary!.rawValue)
                    .font(NubiFont.heading).foregroundColor(selectedPrimary!.color)
            }
            .padding(.horizontal, 20).padding(.vertical, 12)
            .background(selectedPrimary!.color.opacity(0.12)).cornerRadius(20)

            VStack(spacing: 12) {
                ForEach(selectedPrimary!.subEmotions) { sub in
                    Button {
                        withAnimation(.spring(response: 0.35)) { selectedSub = sub }
                        animateNubi = true
                        DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) { animateNubi = false }
                    } label: {
                        HStack(spacing: 14) {
                            ZStack {
                                Circle()
                                    .fill(selectedPrimary!.color.opacity(0.12))
                            ...
                            Image(systemName: "chevron.right")
                                .font(.system(size: 14)).foregroundColor(.nubiGlaucous.opacity(0.5))
                        }
                        .padding(16)
                        .background(Color.white.opacity(0.8)).cornerRadius(16)
    ...
    var confirmationView: some View {
        VStack(spacing: 24) {
            ...
            VStack(spacing: 8) {
                ...
                HStack(spacing: 6) {
                    Image(systemName: selectedPrimary!.sfSymbol)
                        .foregroundColor(selectedPrimary!.color)
                    Text(selectedPrimary!.rawValue)
                        .font(NubiFont.body).foregroundColor(.nubiDark.opacity(0.7))
                    Text("·")
            ...
            .padding(20).nubiCard().padding(.horizontal, 32)

            Text("Nubi tomo nota de como te sientes hoy.\nEsta info enriquecera tu reporte semanal.")
                .font(NubiFont.caption).foregroundColor(.nubiDark.opacity(0.55))
                .multilineTextAlignment(.center).padding(.horizontal, 32)

            Button {
                if let p = selectedPrimary, let s = selectedSub {
                    vm.registerEmotion(primary: p, sub: s)
                }
                dismiss()
            } label: {
                HStack(spacing: 8) {
            ...
            Button {
                withAnimation(.spring()) { selectedSub = nil; selectedPrimary = nil }
            } label: {
                Text("Cambiar mi emocion")
                    .font(NubiFont.caption).foregroundColor(.nubiGlaucous)
            }
        }
    }
